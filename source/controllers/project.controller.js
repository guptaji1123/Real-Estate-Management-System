...
async function createProject(request, response) {
    try {
        ...
        const isExists = await projectModel.findOne({ name })
        if (isExists) {
            return response.status(422).json({ message: 'Project name already exists' })
        }
        const project = await projectModel.create({ name, description, location, status })
...
async function updateProject(request, response) {
    try {
        ...
        const { name, description, location, status } = request.body
        const project = await projectModel.findById(_idProject)
        if (!project) {
        ...
        if (name !== undefined) {
            ...
            const isExists = await projectModel.findOne({ name, _id: { $ne: project._id } })
            if (isExists) {
                return response.status(422).json({ message: 'Project name already exists' })
            }
            project.name = name
        ...
        await project.save()
        return response.status(200).json({ message: 'Project updated successfully', project })
    } catch (error) {
        return response.status(500).json({ message: 'Internal Server Error' })
    }
}
...
async function getProjectsByStatus(request, response) {
    try {
        ...
        const allowedStatus = ['PLANNING', 'ONGOING', 'COMPLETED', 'ON_HOLD']
        if (!allowedStatus.includes(status)) {
            return response.status(400).json({ message: 'Invalid project status' })
        }
...
async function deleteProject(request, response) {
    try {
        const _idProject = request.params._idProject
        const project = await projectModel.findByIdAndDelete(_idProject)
        if (!project) {
            return response.status(404).json({ message: 'Project not found' })
        }
