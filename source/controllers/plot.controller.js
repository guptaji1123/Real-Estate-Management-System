...
async function createPlot(request, response) {
    try {
        ...
        const isExists = await plotModel.findOne({ project: _idProject, plotNumber })
        if (isExists) {
            return response.status(422).json({ message: 'Plot name already exists' })
        }
        const plot = await plotModel.create({ project: _idProject, plotNumber, description, area, unit, price, status })
...
async function updatePlot(request, response) {
    try {
        ...
        await plot.save()
        return response.status(200).json({ message: 'Plot updated successfully', plot })
    } catch (error) {
        return response.status(500).json({ message: 'Internal Server Error' })
    }
}
...
async function search(request, response) {
    try {
        const { _idProject } = request.query
...
async function getPlots(request, response) {
    try {
        ...
        const plots = await plotModel.find({ project: _idProject })
        if (plots.length === 0) {
            return response.status(404).json({ message: 'Plots not found' })
        }
        return response.status(200).json({ plots })
...
async function getPlotsByStatus(request, response) {
    try {
        ...
        const plots = await plotModel.find({ project: _idProject, status })
        if (plots.length === 0) {
            return response.status(404).json({ message: 'Plots not found' })
        }
        return response.status(200).json({ plots })
...
async function deletePlot(request, response) {
    try {
        const { _idPlot, _idProject } = request.params
        const plot = await plotModel.findOneAndDelete({ project: _idProject, _id: _idPlot })
        if (!plot) {
            return response.status(404).json({ message: 'Plot not found' })
        }
        return response.status(200).json({ message: 'Plot Deleted Successfully' })
    } catch (error) {
        return response.status(500).json({ message: 'Internal Server Error' })
    }
}
